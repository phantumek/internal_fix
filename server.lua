...
local function raw_fix(name)
    ...
    if (__resource) then
        ...
        print(SaveResourceFile(name, 'fxmanifest.lua', __resource, -1) and
                  ('[InternalAC-FIX] -> Fixed OLD resource [%s]'):format(name) or
                  ('[InternalAC-FIX] -> Cannot fix [%s]'):format(name));
...
local function fix(n)
    if n then
        ...
        for resources = 0, GetNumResources() - 1 do
            raw_fix(GetResourceByFindIndex(resources));
        end
    end
end

local function show_fix(n)
    local __resource = LoadResourceFile(name, '__resource.lua');
...
local function show(n)
    if n then
        ...
        for resources = 0, GetNumResources() - 1 do
            show_fix(GetResourceByFindIndex(resources))
        end
    end
end

RegisterCommand('fix', function(s, a)
    if s ~= 0 then
        return
    end
